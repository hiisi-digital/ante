...
{
  "imports": {
    "@hiisi/ante": "jsr:@hiisi/ante@^0.1.0"
  }
}
```
...
deno run -A jsr:@hiisi/ante/cli init

# Check if files have valid headers (exits non-zero if issues found)
deno run -A jsr:@hiisi/ante/cli check

# Fix all headers to match config
deno run -A jsr:@hiisi/ante/cli fix

# Add header to a specific file
deno run -A jsr:@hiisi/ante/cli add src/new-file.ts
```
...
```typescript
import { generateHeader, hasValidHeader, loadConfig, parseHeader, resolveConfig } from "@hiisi/ante";
